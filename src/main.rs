...
fn handle_input(root: &mut Root, player: &mut Player, map: &Map){
    ...
    match key{
        Key{code: KeyCode::NumPad8, ..} => player.move_cell(Direction::N, map),
        Key{code: KeyCode::NumPad2, ..} => player.move_cell(Direction::S, map),
        Key{code: KeyCode::NumPad6, ..} => player.move_cell(Direction::E, map),
        Key{code: KeyCode::NumPad4, ..} => player.move_cell(Direction::W, map),
        Key{code: KeyCode::NumPad7, ..} => player.move_cell(Direction::NW, map),
        Key{code: KeyCode::NumPad9, ..} => player.move_cell(Direction::NE, map),
        Key{code: KeyCode::NumPad3, ..} => player.move_cell(Direction::SE, map),
        Key{code: KeyCode::NumPad1, ..} => player.move_cell(Direction::SW, map),
        _ => {},
    }
}
